...
- For received Lightning LNURL payments, use the refreshed Breez `details.inner.lnurlReceiveMetadata.senderComment` field as the Comment row. Do not display raw LNURL metadata; absent, blank, malformed, or oversized SDK comments remain hidden. Historical received comments appear only when that field is retained by Breez.
...
- `PendingBalanceRow` is a presentation shell: it expands/collapses height, opacity, and spacing, stays interactive until exit begins, and normally starts terminal collapse with the matching Pending toast handoff. If an authoritative refresh removes every pending payment without a live Pending handoff for a payment in the last authoritative row (for example, a missed SDK terminal event), it must use the same short collapse rather than retaining stale copy; an unrelated Pending toast must not preserve it. It represents the whole authoritative pending set, so a terminal handoff for one payment never collapses the row while another payment remains pending. While a terminal exit is running, it retains only the last non-empty rendered label so its sats amount does not disappear before unmount; a new authoritative pending set immediately supersedes that snapshot. Reduced-motion uses the same short non-spring transition.
