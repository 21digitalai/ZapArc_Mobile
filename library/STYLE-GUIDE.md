...
- `PendingBalanceRow` is a presentation shell: it expands/collapses height, opacity, and spacing, stays interactive until exit begins, and only starts its terminal collapse with the matching Pending toast handoff. Reduced-motion uses the same short non-spring transition.
